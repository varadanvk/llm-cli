...
def execute_terminal_command(command: str, capture_output: bool = True) -> Dict[str, Any]:
    """
...
def display_command_results(result: Dict[str, Any]) -> None:
    ...
    print(colored(f"Command {status} (exit code: {result['return_code']})", status_color))
...
def web_search(
    query: str, 
    api_key: str, 
    result_count: int = DEFAULT_SEARCH_COUNT
) -> List[Dict[str, Any]]:
    """
    ...
    """
    if not api_key:
        print(colored("Error: No Serper API key provided. Run 'lmci setup' to configure.", "red"))
        return []
